...
# demographics = True
# filterSpecJson = ''
# col filtering, it is a map from col -> s3 urls files containing values for that column
# colValFiltersEnabled = True
# colValFilters = {}
# colValFilters['userID'] = '/mnt/mappr-temp/bombfell_for_vish.csv'
#
# # aditya's access keys for s3 work
# ACCESS_KEY = ""
# SECRET_KEY = ""
...
print 'filteredParquetPath: %s' % filteredParquetPath
...
try:
  ...
  print "Unable to filter. got exception:", e
  filter_spec = None

# a summary of ops to perform
summaryOps = {}
summaryOps['sum'] = ['spend']
summaryOps['countDistinct'] = ['userID', "orderID", 'category', 'brand']
# topCategory / subCategory are auto generated
summaryOps['col_as_tags'] = ['brand', 'category']
summaryOps['logs'] = ['brand', 'category', 'userID', 'spend', 'orderID', 'projWt']
...
def filter_transactions(df, filterSpec):
  ...
  if row_filter:
    print "using row_filter:", row_filter
...
def dateVal(x):
  return float(x[:4]) + float(x[5:7])/12 + float(x[8:10])/365 if len(x) > 0 else -1.0
...
def build_order_tbl(df):
  userAggExpr = [F.first(x).alias(x) for x in userCols if x != entity]
  ...
  orderInfoDF = df.groupBy(entity, orderID).agg(*aggExpr)

  orderInfoDF.registerTempTable('entity_order_tbl')
...
def generate_demographic_info(orderInfoDF, userCols):
  aggExpr = [F.first(x).alias(x) for x in userCols if x not in [userIdCol, entity]]
  aggExpr.append(F.first(weightCol).alias(weightCol))
  orderInfoDF.printSchema()
  print "aggregating for entityUserDF", [x for x in userCols if x not in [userIdCol, entity]]
  entityUserDF = orderInfoDF.groupby(entity, userIdCol).agg(*aggExpr).cache()
  #for categorical
  #calc global freq
...
sqlContext.clearCache()
df = load_data(srcFilePath, data_table)
df.printSchema()
...
if colValFiltersEnabled:
  filtersApplied = True
  print "colValFilters Enabled, only including values.."
  df = filter_by_file(df, colValFilters)

# apply filtering opts
if filter_spec:
  filtersApplied = True
  print "filtering transactions.."
  df = filter_transactions(df, filter_spec)

df = df.repartition(entity).cache()
if filtersApplied and filteredParquetPath:
  df.write.partitionBy(entity).mode("overwrite").parquet('/mnt/%s/%s' %(ETL_BUCKET, filteredParquetPath) , mode="overwrite")
  print "filteredParquetPath data written to: %s" % filteredParquetPath

orderUserInfoDF = build_order_tbl(df).cache()
