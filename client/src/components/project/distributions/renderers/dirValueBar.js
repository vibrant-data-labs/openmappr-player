...
.directive('dirValueBar', ['$timeout', '$q', 'FilterPanelService', 'dataGraph', 'AttrInfoService', 'BROADCAST_MESSAGES',
function($timeout, $q, FilterPanelService, dataGraph, AttrInfoService, BROADCAST_MESSAGES) {
    ...
    **************************************/
    ...
    function renderDistr (scope, element, attrs, renderCtrl) {
        ...
        console.assert(attrInfo.isNumeric && !attrInfo.isTag, 'attribute type should be valid for dirValueBar');
        ...
        var bar = new window.mappr.stats.distr.NumericValueBar(distrSvg, scope.attrToRender, attrInfo, distrOpts);
    ...
    function updateHighlights (bar, scope, renderCtrl) {
        ...
        bar.renderCurrentSel(FilterPanelService.getColorString(), hasValues ? cs : []);
    }
